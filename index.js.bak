...
// ➕ ADD TO UNSOLD (multi-line support)
if (data === "add_unsold") {
    if (!isAdmin) return bot.sendMessage(chatId, "🚫 Admin only.");

    bot.sendMessage(chatId, "📩 Send account details (one per line) in this format:\n`email@example.com password`", { parse_mode: "Markdown" });
    ...
    bot.once("message", async (msg) => {
        ...
        bot.sendMessage(chatId, `✅ Added ${added} account(s) to Unsold Stock.`);
    });
}
