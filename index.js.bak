...
bot.on("callback_query", async (query) => {
  ...
  if (data === "userlist") {
  const res = await db.query('SELECT user_id, username, expires FROM authorized_users');
const entries = res.rows;
const list = entries.length
  ? entries.map(u => `👤 @${u.username || "unknown"} (ID: ${u.user_id})\n⏳ Expires: ${u.expires}`).join("\n\n")
  : "👥 No authorized users.";
...
if (data === "remove_user") {
  ...
  bot.once("message", async (msg) => {
    ...
    const keyRes = await db.query(
      'SELECT duration_months, expires, used FROM license_keys WHERE key_text = $1',
      [key]
    );
    ...
    if (keyData.used) {
      return bot.sendMessage(chatId, "⚠️ This key has already been used.");
    }
    ...
    await db.query(
      `INSERT INTO authorized_users (user_id, username, expires)
       ...
       ON CONFLICT (user_id)
       DO UPDATE SET username = EXCLUDED.username, expires = EXCLUDED.expires`,
      [userId, username, keyData.expires]
    );

    await db.query(
      'UPDATE license_keys SET used = true WHERE key_text = $1',
      [key]
    );

    return bot.sendMessage(chatId, `✅ Key redeemed successfully!\nValid for: ${keyData.duration_months} months\nExpires on: ${keyData.expires}`);
  });
}
  ...
  const res = await db.query(
    'SELECT 1 FROM gmail_store WHERE user_id = $1',
    [userId]
  );

  if (res.rowCount > 0) {
    await db.query('DELETE FROM gmail_store WHERE user_id = $1', [userId]);
  ...
  'SELECT email, password FROM gmail_store WHERE user_id = $1',
  [userId]
);
let info = res.rows[0];

// अगर user का Gmail नहीं है, तो admin का Gmail लो
if (!info) {
  res = await db.query(
    'SELECT email, password FROM gmail_store WHERE user_id = $1',
    [ADMIN_IDS[0]]
